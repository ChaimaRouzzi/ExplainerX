...
const MLRModelBuilder = () => {
  const [plotData2, setPlotData2] = useState([]);
  const [showAlert, setShowAlert] = useState(false);
  const [showAlert2, setShowAlert2] = useState(false);
  const [savingModel, setSavingModel] = useState(false);
  const [mlrParameters, setMlrParameters] = useState({
    model_name: '',
    predictors: [],
    all_predictors: [],
    target: 'Electricity',
  ...
  const handleParameterChange = (event) => {
    ...
    setMlrParameters((prevParameters) => ({
      ...prevParameters,
      [name]: value
    }));
  };
  ...
  const handleSubmit = async (event) => {
    if (selectedPredictors.length !== 0 && mlrParameters.model_name !== '') {
      event.preventDefault();
      setLoading(true);

      const updatedMlrParameters = {
        ...mlrParameters,
      ...
      try {
        const response = await axios.post('http://localhost:8000/api/models_building/build_mlr_model/', updatedMlrParameters, {
          headers: {
  ...
  const handleSaveModel = async () => {
    ...
    const modelToSave = {
      model_name: mlrParameters.model_name,
      user_id: userID,
      params: updatedMlrParameters,
      performance: {
        mse,
        rmse,
        mae
    ...
    try {
      console.log('hi');
      const response = await axios.post('http://localhost:8000/api/models_building/save_model_mlr/', modelToSave, {
        headers: {
  ...
  useEffect(() => {
    if (selectedPredictors.length === 0 || mlrParameters.model_name === '') {
      ...
      setShowAlert2(mlrParameters.model_name === '');
    } else {
      setShowAlert(false);
      setShowAlert2(false);
    }
  }, [selectedPredictors, mlrParameters.model_name]);
  return (
    <div>
            ...
            <Grid container spacing={2}>
              <Grid item xs={6}>
                <FormControl fullWidth variant="outlined" style={verticalSpacing}>
                  ...
                  <Select name="target" value={mlrParameters.target} onChange={handleParameterChange} label="Prediction Target">
              ...
              <Grid item xs={6}>
                <FormControl fullWidth variant="outlined" style={verticalSpacing}>
                  ...
                  <Select name="horizon" value={mlrParameters.horizon} onChange={handleParameterChange} label="Horizon">
            ...
            <Grid container spacing={2} style={verticalSpacing}>
              <Grid item xs={12}>
                <TextField
                  ...
                  label="Model Name"
                  variant="outlined"
                  value={mlrParameters.model_name}
...
export default MLRModelBuilder;
